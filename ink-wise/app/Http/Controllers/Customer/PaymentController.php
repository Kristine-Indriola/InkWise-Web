...
class PaymentController extends Controller
{
    ...
    public function createGCashPayment(Request $request)
    {
        ...
        $outstanding = round(max($order->grandTotalAmount() - $paidAmount, 0), 2);

        \Log::info('GCash Payment Create', [
            'order_id' => $order->id,
            'amount' => $amount,
            'mode' => $mode,
        ...
        if ($mode === 'half' && $amount > $outstanding + 0.01) {
            return response()->json([
                'success' => false,
        ...
        if ($mode === 'full' && $amount > $outstanding + 0.05) {
            return response()->json([
                'success' => false,
                'message' => 'Full payment amount cannot exceed outstanding balance.'
            ], 400);
        }
