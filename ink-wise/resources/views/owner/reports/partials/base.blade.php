@php
  ...
  $tableHeaders = $tableConfig['headers'] ?? ['Report Name', 'Description', 'Generated By', 'Date Generated', 'Action'];
    ...
    <div class="page-inner owner-dashboard-inner" @if($salesShellId) id="{{ $salesShellId }}" @endif>
      ...
      <section class="materials-toolbar" aria-label="Report filters and actions">
        <div class="materials-toolbar__search">
          ...
          <select id="reportFilterSelect" name="range" aria-label="Filter reports by range">
            ...
            <option value="daily" {{ $activeRange === 'daily' ? 'selected' : '' }}>Today</option>
      ...
      @if(!empty($charts))
        <div class="reports-charts">
          @foreach($charts as $chart)
            @php
              $chartId = $chart['id'] ?? ('chart-' . $loop->index);
              $chartPayload = $chart['series'] ?? null;
            @endphp
            <div class="chart-card">
              <h4>{{ $chart['title'] ?? 'Chart' }}</h4>
              <canvas id="{{ $chartId }}" data-chart='@json($chartPayload)' aria-label="{{ $chart['title'] ?? 'Chart' }}" role="img"></canvas>
            </div>
          @endforeach
        </div>
      @endif
      ...
      <div class="table-section-header">
        <div class="table-section-heading">
          <h3>{{ $tableTitle }}</h3>
          @if($tableSubtitle)
            <p class="table-section-subtitle">{{ $tableSubtitle }}</p>
          @endif
        </div>
      </div>
          ...
          <tbody>
            @forelse($tableRows as $row)
              @php
                ...
                $rowDate = $row['date_iso'] ?? null;
                $rowDateIndex = $row['date_index'] ?? null;
              @endphp
              <tr @if($rowDate) data-date="{{ $rowDate }}" @endif @if(!is_null($rowDateIndex)) data-date-index="{{ (int) $rowDateIndex }}" @endif>
                @foreach($rowColumns as $column)
                  @php
                    $cellHtml = $column['html'] ?? null;
                    $cellText = $column['text'] ?? '—';
                  @endphp
                  <td @class([
                    $column['class'] ?? null,
                    'table-cell-muted' => !empty($column['muted']),
                    'table-cell-emphasis' => !empty($column['emphasis']),
                    'table-cell-numeric' => !empty($column['numeric']),
                  ])>
                    @if(!is_null($cellHtml))
                      {!! $cellHtml !!}
                    @else
            ...
            @empty
              @if($tableShowEmpty)
                <tr class="table-empty-row">
                  <td colspan="{{ max(count($tableHeaders), 1) }}">{{ $tableEmptyText }}</td>
                </tr>
              @endif
            @endforelse
          </tbody>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  try {
    ...
    chartCanvases.forEach((canvas) => {
      ...
      try {
        ...
        console.warn('Invalid chart configuration', error);
        return;
      }

      if (!parsedConfig || !Array.isArray(parsedConfig.labels) || !Array.isArray(parsedConfig.datasets)) {
        return;
      }

      const ctx = canvas.getContext('2d');
      const type = parsedConfig.type || 'line';

      const datasets = parsedConfig.datasets.map((dataset) => ({
        tension: 0.25,
        fill: false,
        borderWidth: 2,
        pointRadius: 3.5,
        pointBackgroundColor: dataset.borderColor || '#2b6cb0',
        ...dataset,
      }));
      ...
      const options = mergeDeep(baseOptions, parsedConfig.options ?? {});
    ...
    if (inventoryCanvas && (!inventoryCanvas.dataset.chart || inventoryCanvas.dataset.chart === 'null')) {
  ...
  function extractRowDate(tr) {
    if (!tr) return '';
    if (tr.dataset) {
      if (tr.dataset.date) return tr.dataset.date;
      if (tr.dataset.dateIso) return tr.dataset.dateIso;
    }
    const cells = tr.cells || [];
    const statedIndex = tr.dataset && tr.dataset.dateIndex ? parseInt(tr.dataset.dateIndex, 10) : -1;
    if (!Number.isNaN(statedIndex) && statedIndex >= 0 && statedIndex < cells.length) {
      return (cells[statedIndex].textContent || '').trim();
    }
    const cellCount = cells.length;
    if (!cellCount) {
      return '';
    }
    return (cells[cellCount - 1].textContent || '').trim();
  }
  ...
  function applyReportFilter(range) {
    ...
    rows.forEach((tr) => {
      const dateText = extractRowDate(tr);
      if (!dateText) {
        tr.style.display = '';
        return;
      }
  ...
  (function initReportFilters() {
    ...
    if (ownerReportsState.activeRange) {
      sel.value = ownerReportsState.activeRange;
    }
  ...
  (function wireGenerateModal() {
    ...
    function filterRowsForExport(range, reportName) {
      ...
      rows.forEach(tr => {
        ...
        const dateValue = extractRowDate(tr);
        let dateMatch = true;
        if (dateValue) {
  ...
  function sortReportsTable(desc = false) {
    ...
    rows.sort((a, b) => {
      const aDate = parseYMD(extractRowDate(a));
      const bDate = parseYMD(extractRowDate(b));
