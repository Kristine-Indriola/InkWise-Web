...
@push('styles')
	...
	<style>
		/* Owner layout adjustments for sidebar and topbar */
		.owner-layout main.ordersummary-admin-page {
			padding-left: 230px; /* Account for fixed sidebar width */
			padding-top: calc(64px + 24px); /* Account for topbar height (64px) + gap (24px) */
		...
		@media (max-width: 768px) {
			.owner-layout main.ordersummary-admin-page {
				padding-left: 0;
				padding-top: calc(64px + 40px); /* More top padding on mobile */
			}
		}

		/* Sidebar collapsed state */
		body.sidebar-collapsed .owner-layout main.ordersummary-admin-page {
...
@php
	...
	try {
		$ordersIndexUrl = route('owner.order.workflow');
	} catch (\Throwable $e) {
		$ordersIndexUrl = url('/owner/order/workflow');
	}

	$ordersBackUrl = $ordersIndexUrl;
									...
									@php
										...
										$breakdown = collect(data_get($item, 'breakdown', []))
											...
											->map(function ($row) use ($quantity, $paperStockValue, $addonValues) {
												...
												if ($paperStockValue && $paperStockValue !== '' && str_contains($row['label'], strtolower((string) $paperStockValue))) {
												...
												foreach ($addonValues as $av) {
													if ($av !== '' && str_contains($row['label'], $av)) {
														$row['quantity'] = $quantity;
														if (isset($row['unit_price']) && is_numeric($row['unit_price'])) {
															$row['total'] = (float) $row['unit_price'] * $quantity;
														} elseif (isset($row['total']) && is_numeric($row['total'])) {
										...
										try {
											...
											if (isset($orderModel) && ($orderModel->id ?? null) && $orderItemId) {
									...
									@foreach($ratingPhotos as $photo)
										@php
											$photoUrl = \Illuminate\Support\Str::startsWith($photo, ['http://', 'https://'])
												? $photo
												: \Illuminate\Support\Storage::disk('public')->url($photo);
										@endphp
	...
	const normalizeGallery = function (input) {
		...
		input.forEach(function (entry) {
			...
			if (typeof entry === 'string') {
				...
				orientation = entry.orientation || entry.side || entry.page || entry.label || '';
