...
<div class="materials-container">
  ...
  <div class="summary-cards">
    <a href="{{ route('staff.materials.index', ['status' => 'all']) }}" class="card total">
        <h3>Total Materials</h3>
        <p>{{ $materials->count() }}</p>
    </a>

    <a href="{{ route('staff.materials.index', ['status' => 'low']) }}" class="card low">
        <h3>Low Stock</h3>
        <p>
    ...
    <a href="{{ route('staff.materials.index', ['status' => 'out']) }}" class="card out">
        <h3>Out of Stock</h3>
        <p>
            {{ $materials->filter(function($m) { 
    ...
    <a href="{{ route('staff.materials.index', ['status' => 'qty']) }}" class="card qty">
    ...
    <form method="GET" action="{{ route('staff.materials.index') }}">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="🔍 Search materials..." class="form-control">
        <button type="submit" class="btn btn-secondary">Search</button>
    </form>
</div>

        <a href="{{ route('staff.materials.create') }}" class="btn btn-primary">➕ Add New Material</a>
    </div>
    ...
    <span class="badge {{ $remarkClass }}">{{ $remark }}</span>
                        ...
                        <td class="actions">
                            <a href="{{ route('staff.materials.edit', $material->material_id) }}" class="btn btn-sm btn-warning">✏️</a>
                            <form action="{{ route('staff.materials.destroy', $material->material_id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
