<?php $__env->startSection('title', 'Manage Order Status'); ?>
...
<main class="order-status-shell">
    <a href="<?php echo e(route('staff.orders.summary', $order->id)); ?>" class="back-link">
    ...
    <section class="order-status-card">
        <header class="order-status-card__header">
            <div>
                ...
                <p class="order-status-card__subtitle">Update the status, tracking information, and notes for order <?php echo e($orderNumber); ?></p>
            </div>
            <span class="order-stage-chip order-stage-chip--<?php echo e($currentChipModifier); ?>">
        ...
        <div class="order-status-meta">
            <div class="order-status-meta__item">
                ...
                <span class="order-status-meta__value"><?php echo e($customerName); ?></span>
            </div>
            <div class="order-status-meta__item">
                <span class="order-status-meta__label">Order Date</span>
                <span class="order-status-meta__value"><?php echo e($placedDateDisplay ?: 'Unknown'); ?></span>
            </div>
            <div class="order-status-meta__item">
                <span class="order-status-meta__label">Last Updated</span>
                <span class="order-status-meta__value"><?php echo e($lastUpdatedDisplay ?: 'Never'); ?></span>
            </div>
        </div>
    </section>
    ...
    <section class="status-progress-card">
        ...
        <ol class="status-tracker" aria-label="Order progress">
            ...
            <?php $__currentLoopData = $statusFlow; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $statusKey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                ?>
                <li class="status-tracker__item <?php echo e(implode(' ', $itemClasses)); ?>">
                    <div class="status-tracker__marker" aria-current="<?php echo e($isCurrent ? 'step' : 'false'); ?>">
                        <?php if($isDone): ?>
                            <i class="fi fi-rr-check"></i>
                        <?php else: ?>
                    ...
                    <h3 class="status-tracker__title"><?php echo e($statusLabel); ?></h3>
                    <?php if($isCurrent && $nextStatusLabel): ?>
                        <div class="status-tracker__content">
                            <p class="status-tracker__subtitle">Next: <?php echo e($nextStatusLabel); ?></p>
                        </div>
                    <?php endif; ?>
                </li>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <?php if($currentStatus !== 'cancelled'): ?>
    <form method="POST" action="<?php echo e(route('staff.orders.status.update', $order)); ?>" class="status-form-card status-form">
        <?php echo csrf_field(); ?>
        ...
        <div class="form-row">
            ...
            <select name="status" id="status" required>
                <?php $__currentLoopData = $statusLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($value); ?>" <?php echo e(old('status', $currentStatus) === $value ? 'selected' : ''); ?>>
                        <?php echo e($label); ?>
...
<?php echo $__env->make('layouts.staffapp', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\leanne\xampp\htdocs\InkWise-Web\ink-wise\resources\views/staff/orders/manage-status.blade.php ENDPATH**/ ?>